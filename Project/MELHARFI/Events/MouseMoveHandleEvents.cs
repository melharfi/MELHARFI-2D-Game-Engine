                    ...
                    for (int cnt = gfxBgrList.Count(); cnt > 0; cnt--)
                    {
                        ...
                        if (gfxBgrList[cnt - 1] != null && gfxBgrList[cnt - 1].GetType() == typeof(FillPolygon))
                        {
                            ...
                            foreach (IGfx t in f.Child)
                            {
                                ...
                                {
                                    Txt childR = t as Txt;
                                    if (f.Visible && childR.Visible && new Rectangle(childR.point.X + f.rectangle.X, childR.point.Y + f.rectangle.Y, TextRenderer.MeasureText(childR.Text, childR.font).Width, TextRenderer.MeasureText(childR.Text, childR.font).Height).Contains(e.Location))
                                    {
                                        if (childR.Visible)
                                        {
                                            SolidBrush sb = childR.brush as SolidBrush;
                ...
                if (MouseOutRecorder.Count <= 0) return;
                {
                    ...
                    for (int cnt = gfxTopList.Count(); cnt > 0; cnt--)
                    {
                        ...
                        else if (gfxTopList[cnt - 1] != null && gfxTopList[cnt - 1].GetType() == typeof(Txt))
                        {
                            ...
                            t.Child.Sort(0, t.Child.Count, rzi);
                            #region parent
                            if (MouseOutRecorder.IndexOf(t) != -1 && (!new Rectangle(t.point, TextRenderer.MeasureText(t.Text, t.font)).Contains(e.Location) || (MouseOverRecorder.Count > 0 && MouseOverRecorder.IndexOf(t) == -1)))
                            {
                                t.FireMouseOut(e);
                    ...
                    for (int cnt = gfxObjList.Count(); cnt > 0; cnt--)
                    {
                        ...
                        else if (gfxObjList[cnt - 1] != null && gfxObjList[cnt - 1].GetType() == typeof(Txt))
                        {
                            ...
                            r.Child.Sort(0, r.Child.Count, rzi);
                            #region parent
                            if (MouseOutRecorder.IndexOf(r) != -1 && (!new Rectangle(r.point, TextRenderer.MeasureText(r.Text, r.font)).Contains(e.Location) || (MouseOverRecorder.Count > 0 && MouseOverRecorder.IndexOf(r) == -1)))
                            {
                                r.FireMouseOut(e);
                                MouseOutRecorder.Remove(r);
                                found = true;
